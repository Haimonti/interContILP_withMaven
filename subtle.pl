:- module(subtle, [lgg/3, reduce/2, theta_subsumes/2, theta_subsumes/3, equiv/2]).
...
% theta_subsumes(+X,+Y) <=> X theta_subsumes Y
% theta_subsumes(+X,+Y,-S) <=> X theta_subsumes Y and S is a substitution that proves this
% equiv(+X,+Y) <=> X theta_subsumes Y and Y theta_subsumes X
...
% returns Sub=true iff A theta_subsumes at least one atom, + returns all atoms that subsume A - those certainly aren't unique
getEquiv(A, [], [], [], false).
getEquiv(A, [B|C], Unifs, RestB, Sub) :-
	theta_subsumes_chk(A,B)
	-> Sub=true, 
	   (theta_subsumes_chk(B,A) -> Unifs=[B|U], RestB=RestC; Unifs=U, RestB=[B|RestC]),
...
redC([A|B], Necessary, RedC) :-
	...
	(theta_subsumes([A|All], All, Subst) 
...
% method for testing whether C theta_subsumes D: 
...
equiv(X,Y) :- theta_subsumes(X,Y,_), theta_subsumes(Y,X,_).

theta_subsumes(X, Y) :- theta_subsumes(X, Y, _).

theta_subsumes(X, Y, Sub) :- 
