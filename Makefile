...
#
YAP_EXTRAS=-DCUT_C=1  -DCOROUTINING=1 -DRATIONAL_TREES=1 -DDEPTH_LIMIT=1
...
# for c_interface.c
#
CC=gcc-7 -fstrict-aliasing -freorder-blocks -fsched-interblock -Wall -Wstrict-aliasing=2
MPI_CC=gcc-7
CPPFLAGS=  -I. -I$(srcdir)/H -I$(srcdir)/OPTYap -I$(srcdir)/BEAM -I$(srcdir)/MYDDAS
EXECUTABLE_CFLAGS= -DBP_FREE -O3 -fomit-frame-pointer -Wall -Wstrict-prototypes -Wmissing-prototypes -g -O2 $(YAP_EXTRAS) $(DEFS) $(CPPFLAGS)
CFLAGS=  -DBP_FREE -O3 -fomit-frame-pointer -Wall -Wstrict-prototypes -Wmissing-prototypes -g -O2 $(YAP_EXTRAS) $(DEFS) $(CPPFLAGS)
C_INTERF_FLAGS= -DBP_FREE -O3 -fomit-frame-pointer -Wall -Wstrict-prototypes -Wmissing-prototypes -g -O2 $(YAP_EXTRAS) $(DEFS) $(CPPFLAGS) -I$(srcdir)/include
C_PARSER_FLAGS= -DBP_FREE -O3 -fomit-frame-pointer -Wall -Wstrict-prototypes -Wmissing-prototypes -g -O2 $(YAP_EXTRAS) $(DEFS) $(CPPFLAGS)
#
#
# You shouldn't need to change what follows.
#
ARCH=i386-darwin13.4.0
AWK=awk
CPP=gcc-7 -E
DEFS=-DHAVE_CONFIG_H -D_YAP_NOT_INSTALLED_=1
LIBS=-lgmp -lresolv -lstdc++ -lm  -lreadline -lncurses -lz
LDFLAGS=
LN_S=ln -s
M4=m4
GCC=yes
...
AR=ar
srcdir=.
abs_top_builddir=/Users/Haimonti/Research/DDM/DistILP/Code/yap-6.2.2
SO=dylib
MAKEINFO=makeinfo
TEXI2DVI=texi2dvi
TEXI2HTML=texi2html
...
ENGINE_OBJECTS = \
	...
	write.o \
	blobs.o swi.o ypsocks.o ypstdio.o 
...
STATIC_OBJECTS = \
	# sys.o yap_random.o regexp.o  # regcomp.o regerror.o regfree.o regexec.o
...
regexp.o: $(srcdir)/library/regex/regexp.c  # $(srcdir)/library/regex/regex2.h  $(srcdir)/library/regex/engine.c config.h
...
all: startup.yss
	 ...
	 (cd library/lammpi; $(MAKE))
	@#  (cd library/matlab; $(MAKE))
	 (cd library/matrix; $(MAKE))
	 (cd library/random; $(MAKE))
	 (cd library/regex; $(MAKE))
	 (cd library/rltree; $(MAKE))
	 (cd library/system; $(MAKE))
	 (cd library/tries; $(MAKE))
	 (cd packages/clib; $(MAKE))
	 (cd packages/http; $(MAKE))
	 (cd packages/plunit; $(MAKE))
	 (cd packages/RDF; $(MAKE))
	 (cd packages/semweb; $(MAKE))
	 (cd packages/sgml; $(MAKE))
	 #(cd packages/swi-minisat2/C; $(MAKE))
	 (cd packages/tai; $(MAKE))
	 (cd packages/zlib; $(MAKE))
	@# (cd packages/cplint/approx/simplecuddLPADs; $(MAKE))
	@# (cd packages/cplint/slipcase; $(MAKE))
	@# (cd packages/cplint; $(MAKE))
	@#  (cd packages/ProbLog/simplecudd; $(MAKE))
	@#  (cd packages/ProbLog/simplecudd_lfi; $(MAKE))
	@#  (cd packages/jpl; $(MAKE))

startup.yss: yap $(PL_SOURCES)
	-rm -f startup.yss
	echo "bootstrap('$(srcdir)/pl/init.yap'). module(user). save_program('startup.yss')." |  DYLD_LIBRARY_PATH=$(abs_top_builddir) ./yap -b $(srcdir)/pl/boot.yap

yap: $(HEADERS) yap.o libYap.a
	$(MPI_CC) $(EXECUTABLE_CFLAGS) $(LDFLAGS) -o yap yap.o libYap.a $(LIBS) 
...
libYap.dylib: $(LIB_OBJECTS)
	gcc-7 -fstrict-aliasing -freorder-blocks -fsched-interblock -Wall -Wstrict-aliasing=2 -dynamiclib -Wl,-install_name,NONE/lib/libYap.dylib -o libYap.a $(LIB_OBJECTS) $(LIBS) $(LDFLAGS) $(SONAMEFLAG)

install: install_bin install_data

install_bin: install_unix
...
install_unix: startup.yss libYap.a
	...
	for f in $(PL_SOURCES); do $(INSTALL) $$f $(DESTDIR)$(SHAREDIR)/Yap/pl; done
	 ...
	 (cd library/lammpi; $(MAKE) install)
	 ...
	 (cd packages/zlib; $(MAKE) install)
	# (cd packages/swi-minisat2/C; $(MAKE) install)
	@#  (cd library/matlab; $(MAKE) install)
	@#  (cd packages/jpl; $(MAKE) install)
	#@#  (cd packages/pyswip; $(MAKE) install)
	...
	@# (cd packages/cplint; $(MAKE) install)
#	@# (cd packages/cplint/approx/simplecuddLPADs; $(MAKE) install)
#	@# (cd packages/cplint/slipcase; $(MAKE) install)
	@#  (cd packages/ProbLog/simplecudd; $(MAKE) install)
	@#  (cd packages/ProbLog/simplecudd_lfi; $(MAKE) install)
...
install_win32: startup.yss
	...
	$(INSTALL_PROGRAM) -m 755 libYap.a $(DESTDIR)$(YAPLIBDIR)/libYap.a
...
install_data: install_copied_files install_bin
	# (cd packages/chr ; $(MAKE) install)
	 (cd packages/clpqr ; $(MAKE) install)
...
clean: clean_docs
	rm -f *.o *~ *.BAK *.a
	 (cd packages/PLStream; $(MAKE) clean)
	 (cd library/lammpi; $(MAKE) clean)
	@#  (cd library/matlab; $(MAKE) clean)
	 (cd library/matrix; $(MAKE) clean)
	 (cd library/random; $(MAKE) clean)
	 (cd library/regex; $(MAKE) clean)
	 (cd library/rltree; $(MAKE) clean)
	 (cd library/system; $(MAKE) clean)
	 (cd library/tries; $(MAKE) clean)
	 (cd packages/clib; $(MAKE) clean)
	 (cd packages/http; $(MAKE) clean)
	 (cd packages/plunit; $(MAKE) clean)
	 (cd packages/RDF; $(MAKE) clean)
	 (cd packages/semweb; $(MAKE) clean)
	 (cd packages/sgml; $(MAKE) clean)
	 (cd packages/swi-minisat2; $(MAKE) clean)
	 (cd packages/tai; $(MAKE) clean)
	 (cd packages/zlib; $(MAKE) clean)
	@# (cd packages/cplint/approx/simplecuddLPADs; $(MAKE) clean)
	@# (cd packages/cplint; $(MAKE) clean)
	@#  (cd packages/ProbLog/simplecudd; $(MAKE) clean)
	@#  (cd packages/ProbLog/simplecudd_lfi; $(MAKE) clean)
	@#  (cd packages/jpl; $(MAKE) clean)



realclean: clean realclean_docs
	rm -f yap startup.yss config.log config.cache config.h
